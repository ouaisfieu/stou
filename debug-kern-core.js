...
(function(global) {
    ...
    function loadAgent() {
        ...
        if (!agent) {
            ...
            if (!agent.stats) {
                agent.stats = defaults.stats;
            } else {
                // Merge missing stats properties
                Object.keys(defaults.stats).forEach(key => {
                    if (agent.stats[key] === undefined) {
                        agent.stats[key] = defaults.stats[key];
                    }
                });
            }
            ...
            if (!agent.skills) {
                ...
                Object.keys(defaults.skills).forEach(key => {
                    if (!agent.skills[key]) {
                        agent.skills[key] = defaults.skills[key];
                    }
                });
            }
            ...
            if (!agent.settings) {
                agent.settings = defaults.settings;
            }
            
            // Ensure other required fields
            if (!agent.xpToNext) agent.xpToNext = 1000;
            if (!agent.level) agent.level = 1;
            if (!agent.xp) agent.xp = 0;
            if (!agent.clearance) agent.clearance = 0;
